...
class StoreOfertaRequest extends FormRequest {
	...
	public function messages(): array {
		return [
			...
			'items.*.equipos.*.equipo_selec.required' => 'El campo :attribute es obligatorio.',
		];
	}
	 public function attributes(): array
    {
        ...
        foreach ($this->items ?? [] as $i => $item) {
            $index = $i + 1; // convertir de base 0 a base 1
            ...
            $attributes["items.$i.equipos.*.equipo_selec"] = "equipo del Item {$index}";
        }
