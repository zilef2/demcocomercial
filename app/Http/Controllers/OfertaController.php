...
// Asegúrate de importar el modelo Cobre
use App\Models\Item;
...
class OfertaController extends Controller {
	...
	public function GuardarNuevaOferta(StoreOfertaRequest $request): RedirectResponse {
		
		Myhelp::EscribirEnLog($this, ' Begin ' . __METHOD__, ' primera linea del metodo ' . __METHOD__);
		$validated = $request->validated();
		
		//correo
		$theuserlogeed = Myhelp::AuthU();
		$mensajeCorreo = $theuserlogeed->name . " ha pasado la validacion de ofertas";
		SendEmailJob::dispatch($mensajeCorreo, 'Han creado una nueva oferta');
	...
	public function buscarEquipos(Request $request): JsonResponse {
		...
		$equipos = Equipo::where('codigo', 'like', "%$query%")->orWhere('descripcion', 'like', "%$query%")->orWhere('codigo', 'like', "%$query%")->limit(52)->get();
		
		return response()->json(Myhelp::MakeSelect_hardmode($equipos, 'Equipo', false, 'codigo', 'descripcion', $this->camposNumericos, $this->camposString));
	}
	...
	public function equiposApiError(Request $request): void {
		...
		SendEmailJob::dispatch(' Error en la API de equipos: ' . $request->get('error', '') . ' Usuario: ' . Myhelp::AuthU()->name . ' IP: ' . $request->ip());
	}
...
'ultra_valor_total' => 'required|numeric|min:0',
	...
	public function guardarFilasCable(Request $request) {
		$validated = $request->validate([
			                                'filas'    => 'present|array',
		...
		try {
			$cableIds = [];
			foreach ($validated['filas'] as $fila) {
				if ($fila['calibre'] !== null) { // Solo guardar si hay un calibre seleccionado
					$cable = \App\Models\Cable::create([
						                                   'item_id'        => $validated['item_id'],
						                                   'descripcion'    => $fila['descripcion'],
						                                   'cantidad'       => $fila['cant'],
						                                   'metros'         => $fila['mts'],
						                                   'calibre'        => $fila['calibre'],
						                                   'valor_unitario' => $fila['vl_unitario'],
						                                   'total'          => $fila['total'],
					                                   ]);
					$cableIds[] = $cable->id;
				}
			...
			return response()->json([
				                        'message'     => 'Filas de cable guardadas correctamente.',
				                        'cableado_id' => $cableIds
			                        ]);
			
		} catch (\Throwable $e) {
			DB::rollBack();
			
			return response()->json(['error' => 'Hubo un error al guardar los datos de cable. ' . $e->getMessage()], 500);
		}
	}
}