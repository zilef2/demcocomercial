...
import Create from '@/Pages/reproceso/Create.vue';
import Edit from '@/Pages/reproceso/Edit.vue';
import Delete from '@/Pages/reproceso/Delete.vue';
...
watch(() => _.cloneDeep(data.params), debounce(() => {
    ...
    router.get(route("reproceso.index"), params, {
        replace: true,
        preserveState: true,
        preserveScroll: true,
    })
...
const selectAll = (event) => {
    if (event.target.checked === false) {
        ...
        props.reprocesos?.data.forEach((reproceso) => {
            data.selectedId.push(reproceso.id)
        })
    }
}
const select = () => {
    if (props.reprocesos?.data.length == data.selectedId.length) {
        ...
        <div class="space-y-4">
            ...
            <div class="px-4 sm:px-0">
                <div class="rounded-lg overflow-hidden w-fit">
                    <PrimaryButton class="rounded-none" @click="data.createOpen = true" v-if="can(['create reproceso'])">
                        {{ lang().button.add }}
                    </PrimaryButton>

                    <Create v-if="can(['create reproceso'])" :numberPermissions="props.numberPermissions" :titulos="titulos"
                        :show="data.createOpen" @close="data.createOpen = false" :title="props.title"
                        :losSelect=props.losSelect />

                    <Edit v-if="can(['update reproceso'])" :titulos="titulos" :numberPermissions="props.numberPermissions"
                        :show="data.editOpen" @close="data.editOpen = false" :generica="data.generico" :title="props.title"
                        :losSelect=props.losSelect />

                    <Delete v-if="can(['delete reproceso'])" :numberPermissions="props.numberPermissions"
                ...
                <div class="flex justify-between p-2">
                    <div class="flex space-x-2">
                        ...
                        <!-- <DangerButton @click="data.deleteBulkOpen = true"
                            v-show="data.selectedId.length != 0 && can(['delete reproceso'])" class="px-3 py-1.5"
