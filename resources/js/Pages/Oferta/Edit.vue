...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.Ofertaa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.Ofertaa[names['order']]
        });

        // form.codigo = props.Ofertaa?.codigo
    }
})

const update = () => {
    form.put(route('Oferta.update', props.Ofertaa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
