...
import Create from '@/Pages/cable/Create.vue';
import Edit from '@/Pages/cable/Edit.vue';
import Delete from '@/Pages/cable/Delete.vue';
...
watch(() => _.cloneDeep(data.params), debounce(() => {
    ...
    router.get(route("cable.index"), params, {
        replace: true,
        preserveState: true,
        preserveScroll: true,
    })
...
const selectAll = (event) => {
    if (event.target.checked === false) {
        ...
        props.fromController?.data.forEach((cable) => {
            data.selectedId.push(cable.id)
        })
    }
}
...
//thisisnew!!!
//float1  // float3 // bigdecimal3 // json

const titulos = [
    { order: 'descripcion', label: 'descripcion', type: 'string' },
    { order: 'cantitdad', label: 'cantitdad', type: 'float3' },
    { order: 'metros', label: 'metros', type: 'float3' },
    { order: 'calibre', label: 'calibre', type: 'string' },
    { order: 'total', label: 'total', type: 'float3' },
    { order: 'campoauxiliar1', label: 'campoauxiliar1', type: 'float3' },
    { order: 'campoauxiliar2', label: 'campoauxiliar2', type: 'float3' },
    { order: 'tipo', label: 'tipo', type: 'string' },
    { order: 'tiponum', label: 'tiponum', type: 'number' },
];
            ...
            <div class="px-4 sm:px-0">
                <div class="rounded-lg overflow-hidden w-fit">
                    <PrimaryButton class="rounded-none" @click="data.createOpen = true"
                        v-if="can(['create cable'])">
                        {{ lang().button.new }}
                    </PrimaryButton>

                    <Create v-if="can(['create cable'])" :numberPermissions="props.numberPermissions"
                        :titulos="titulos" :show="data.createOpen" @close="data.createOpen = false" :title="props.title"
                        :losSelect=props.losSelect />

                    <Edit v-if="can(['update cable'])" :titulos="titulos"
                        :numberPermissions="props.numberPermissions" :show="data.editOpen" @close="data.editOpen = false"
                        :cablea="data.cableo" :title="props.title" :losSelect=props.losSelect />

                    <Delete v-if="can(['delete cable'])" :numberPermissions="props.numberPermissions"
                        :show="data.deleteOpen" @close="data.deleteOpen = false" :cablea="data.cableo"
                        :title="props.title" />
                </div>
            </div>
                ...
                <div class="flex justify-between p-2">
                    <div class="flex space-x-2">
                        ...
                        <!-- <DangerButton @click="data.deleteBulkOpen = true"
                            v-show="data.selectedId.length != 0 && can(['delete cable'])" class="px-3 py-1.5"
                        ...
                        <tbody>
                            <tr v-for="(claseFromController, indexu) in props.fromController.data" :key="indexu"
                                ...
                                <td v-if="numberPermissions > 1" class="whitespace-nowrap py-4 w-12 px-2 sm:py-3">
                                    <div class="flex justify-center items-center">
                                        <div class="rounded-md overflow-hidden">
                                            <InfoButton v-show="can(['update cable'])" type="button"
                                                @click="(data.editOpen = true), (data.cableo = claseFromController)"
                                                class="px-2 py-1.5 rounded-none" v-tooltip="lang().tooltip.edit">
                                                <PencilIcon class="w-4 h-4" />
                                            </InfoButton>
                                            <DangerButton v-show="can(['delete cable'])" type="button"
                                                @click="(data.deleteOpen = true), (data.cableo = claseFromController)"
                                ...
                                <td v-for="titulo in titulos" class="whitespace-nowrap py-4 px-2 sm:py-3">
                                    ...
                                    <span v-if="titulo['type'] === 'number' || titulo['type'] === 'bigdecimal3'"> {{ number_format(claseFromController[titulo['order']], 0, false) }} </span>
                                    <span v-if="titulo['type'] === 'dinero'"> {{ number_format(claseFromController[titulo['order']], 0, true) }} </span>
                                    <span v-if="titulo['type'] === 'float1'"> {{ number_format(claseFromController[titulo['order']], 0, true) }} </span>
                                    <span v-if="titulo['type'] === 'float3'"> {{ number_format(claseFromController[titulo['order']], 0, true) }} </span>
