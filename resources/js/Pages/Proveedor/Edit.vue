...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.Proveedora[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.Proveedora[names['order']]
        });

        // form.codigo = props.Proveedora?.codigo
    }
})

const update = () => {
    form.put(route('Proveedor.update', props.Proveedora?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
